...
export default (props) => {
    ...
    useEffect(() => {
        Axios.get(`${BASE_URL}/public/merchandises/${merchandise_id}/`).then(
            response => {
                    ...
                    <List subheader={<ListSubheader disableSticky> Choose one option (required) </ListSubheader>}>
                        {services.map(service => (
                            <ListItem button onClick={() => {
                                ctx.dispatch({type: "MERCHANDISE_SERVICE_SET", payload: service});
                                if (service.address_required) {
                                    history.replace({pathname: "/chooseAddress", hidden: false})
                                } else {
                                    history.replace("/checkout")
                                }
                            }}>
                                <ListItemText> <b> {service.name} </b> </ListItemText>
                                <ListItemSecondaryAction>
                                </ListItemSecondaryAction>
                            </ListItem>
                        ))}
